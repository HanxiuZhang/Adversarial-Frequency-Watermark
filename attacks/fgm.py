...
def fgm_direct(img: Tensor, label: Tensor, wm: Tensor, model: nn.Module, alpha:float, beta: float,block_size: int=8,**args) -> Tensor:
    r'''
...
def fgm_wm_opti(img: Tensor, label: Tensor, wm: Tensor, model: nn.Module, 
                ...
                N: int, l1: float, l2: float, s_a: float, s_b: float, beta_max: float,**args) -> Tensor:
    r'''
    ...
    if(check_out(model,wmed,label)):
        ...
        if(check_out(model,wmed,label)):
            ...
            return wmed_res,wm_extracted, alpha_in, beta_in  # type: ignore
    ...
    for _ in range(N):
        ...
        if check_out(model,wmed,label):
            wm_extracted = extract_wm(img,wmed_res,alpha,block_size)
            return wmed_res,wm_extracted, alpha, beta  # type: ignore
        else:
